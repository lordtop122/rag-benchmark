...
from retrievers import RETRIEVER_REGISTRY, DenseRetriever
...
def main():
    ...
    # Load the embedding model once and reuse it across all chunk sizes --
    # loading a neural network model repeatedly is slow and unnecessary,
    # since the same model can encode chunks of any size.
    print("Loading sentence-transformer model for dense retrieval...")
    dense_model = SentenceTransformer(DenseRetriever.MODEL_NAME)
    ...
    for chunk_size in CHUNK_SIZES:
        ...
        for method_name, retriever_cls in RETRIEVER_REGISTRY.items():
            if method_name == "dense":
                retriever = retriever_cls(chunks, model=dense_model)
            else:
